...
const {delete_all} = require('./statusController')
...
describe('Test the root of the status api', () => {
  ...
  test('it should respond to the POST method', (done) => {
    request(app).post('/api/status').send({
        name: "Test Section"
      }).then((response) => {
      expect(response.statusCode).toBe(200);
...
describe('Test the specific sections of the status api', () => {
  ...
  test('it should respond to the POST method', (done) => {
    request(app).post('/api/status/'+id).send({
      name: "New Name",
      status: "closed"
    }).then((response) => {
      expect(response.statusCode).toBe(200);
  ...
  test('it should respond to the DELETE methid', (done) => {
    request(app).delete('/api/status/'+id).then((response) => {
      expect(response.statusCode).toBe(200);
      expect(response.body['message']).toBe("Section Status successfully deleted");
